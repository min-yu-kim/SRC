...
def grid_generate_bound(rotated_coords, x_grid, y_grid, grid_size):
    ...
    for coord in rotated_coords:
        ...
        nearest_grid_x = min(filter(lambda x: x <= start_x, x_grid), key=lambda x: abs(x - start_x))
        nearest_grid_y = min(filter(lambda y: y <= start_y, y_grid), key=lambda y: abs(y - start_y))
    ...
    grid_coords = []
    # 그리드 생성
    for i in range(int((max(x) - nearest_grid_x) / dx)+1):
        for j in range(int((max(y) - nearest_grid_y) / dy)+1):
            # 그리드 좌표 계산
            x1 = nearest_grid_x + i * dx
            x2 = nearest_grid_x + (i + 1) * dx
            y1 = nearest_grid_y + j * dy
            y2 = nearest_grid_y + (j + 1) * dy
...
def grid_rotate_to_origin(rotated_coords, grid_coords, theta):
    ...
    for grid in grid_coords:
        ...
        rotated_coords += pivot
        # rotated_points.append(rotated_coords)
...
def convert_to_xywh(rectangle_coordinates_list):
    rectangle_coordinates = rectangle_coordinates_list[0]

    x_values = [int(point[0]) for point in rectangle_coordinates]
    y_values = [int(point[1]) for point in rectangle_coordinates]

    x_start = min(x_values)
    y_start = min(y_values)
    width = max(x_values) - x_start
    height = max(y_values) - y_start

    return [x_start, y_start, width, height]

def transform_coordinates(input_coordinates):
    # 입력된 좌표를 변환할 빈 리스트 생성
    output_coordinates = []

    # 입력된 좌표를 순회하면서 변환
    for sublist in input_coordinates:
        for sublist_2 in sublist:
            for pair in sublist_2:
                # 변환된 좌표를 튜플로 만들어 리스트에 추가
                output_coordinates.append(tuple(pair))

    return set(output_coordinates)
...
def get_bounding_box(vertices):
    ...
    bound_box = np.array(min_x, min_y, max_x, max_y)
