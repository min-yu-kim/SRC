    ...
    for index2 in range(index1 + 1, len(open_line_segments)):
        ...
        if (done[0] is False):
            if (double_check):
                ...
                lines_to_check.append(
                    [centroid([curr_segment[0], curr_vertex]), centroid([next_segment[0], next_vertex]), 0]);
                lines_to_check.append(
                    [centroid([curr_segment[0], curr_vertex]), centroid([next_segment[1], next_vertex]), 0])
        ...
        if (done[1] is False):
            if (double_check):
                ...
                lines_to_check.append(
                    [centroid([curr_segment[1], curr_vertex]), centroid([next_segment[0], next_vertex]), 1])
                lines_to_check.append(
                    [centroid([curr_segment[1], curr_vertex]), centroid([next_segment[1], next_vertex]), 1])
...
if (boundary[0].x != new_sorted_vertices[0].x):
    quad_cells.append(
        [boundary[0], point(new_sorted_vertices[0].x, y_limit_lower), point(new_sorted_vertices[0].x, y_limit_upper),
         boundary[3]])
...
for index1 in range(len(quad_cells)):
    ...
    for index2 in range(len(quad_cells)):
        if (index1 != index2):
            if ((quad_cells[index1][1].x == quad_cells[index2][0].x) and (
                    (quad_cells[index1][2].y in [quad_cells[index2][0].y, quad_cells[index2][3].y]) or (
                    quad_cells[index1][1].y in [quad_cells[index2][0].y, quad_cells[index2][3].y]))):
...
for index in range(len(graph_vertices)):
    str_to_write = str_to_write + ", " + str(index) + ":" + "(" + str(int(graph_vertices[index].x)) + ", " + str(
        int(graph_vertices[index].y)) + ")"
